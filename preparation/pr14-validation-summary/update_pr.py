...
ROOT=Path('WORKSPACE');E=ROOT/'extras/batch-evidence/pr14-validation-summary'
...
HEAD='8d456962f0bd410b4a548969cfb547f39ede9f18'
PREVIOUS='6b9540afff64c28e65ee3b55aaf70c1b6ce94a21'
...
dest=PUB/'preparation/pr14-validation-summary';dest.mkdir(parents=True)
index=[]
for name in ('final-checks.json','final-checks.log','docs-check.json','summary-inputs.json','verify.py','docs_check.py','update_pr.py'):
...
run(['git','-C',str(PUB),'add','.']);run(['git','-C',str(PUB),'commit','-m','docs: retain evidence for the simplified validation summary'])
archive=run(['git','-C',str(PUB),'rev-parse','HEAD']).strip()
assert run(GIT+['ls-remote','origin','refs/heads/evidence/vs2-interaction-r2']).split()[0]=='338c95be023bca7ab0f36779bed29882762ca6ad'
...
url=f'https://github.com/FinnNk/Meerkritic/blob/{archive}/preparation/pr14-validation-summary/'
...
new_row='| `'+HEAD[:7]+'` | Summarise standard checks once and reserve tables for useful test and coverage detail. |\n'
body=body.replace('\n\n## Validation\n','\n'+new_row+'\n## Validation\n',1)
