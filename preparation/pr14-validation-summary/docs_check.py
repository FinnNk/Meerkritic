...
ROOT = Path('WORKSPACE')
R = ROOT/'extras/der-checkouts/vs2-study-preparation'
E = ROOT/'extras/batch-evidence/pr14-validation-summary'
OLD = 'f7aa4e05411da9d804f3624abe57033544fb1073'
git = ['git','-c',f'safe.directory={R.as_posix()}','-c',f'safe.directory={(ROOT/"working").as_posix()}','-C',str(R)]
revision = subprocess.check_output(git+['rev-parse','HEAD'],text=True).strip()
assert not subprocess.check_output(git+['status','--porcelain'],text=True).strip()
md = MarkdownIt('commonmark').enable('table')
files = [R/p for p in subprocess.check_output(['git','-c',f'safe.directory={R.as_posix()}','-c',f'safe.directory={(ROOT/"working").as_posix()}','-C',str(R),'ls-files','*.md'], text=True).splitlines()
...
manifest = json.loads((R/'docs/source-manifest.json').read_text(encoding='utf-8'))
for item in manifest['files']:
    if hashlib.sha256((R/item['path']).read_bytes()).hexdigest() != item['sha256']:
        errors.append('Changed imported original: '+item['path'])
changed = subprocess.check_output(['git','-c',f'safe.directory={R.as_posix()}','-c',f'safe.directory={(ROOT/"working").as_posix()}','-C',str(R),'diff','--name-only',OLD,'HEAD'],text=True).splitlines()
assert not any(p.startswith(('src/','tools/','tests/')) and p != 'tests/README.md' for p in changed)
assert 'uv.lock' not in changed
project = tomllib.loads((R/'pyproject.toml').read_text(encoding='utf-8'))['project']
assert project['license'] == 'MIT' and project['license-files'] == ['LICENSE.md']
assert not (R/'LICENSE').exists()
assert 'Copyright (c) 2026 Finn Newick' in (R/'LICENSE.md').read_text(encoding='utf-8')
...
for args in commands:
    result = subprocess.run([str(ROOT/'extras/der-checkouts/pr13-integration-merged/.venv/Scripts/python.exe'), *args], cwd=R,
...
samples = ['docs/development/study-preparation.md','docs/edr/0001-discovery-grouping-method.md','README.md','docs/development/normalisation.md','docs/development/research-interaction.md',
           'docs/development/documentation-style.md','docs/edr/README.md']
render = E/'rendered';render.mkdir(exist_ok=True)
for name in samples:
    content = md.render((R/name).read_text(encoding='utf-8'))
...
assert not subprocess.check_output(git+['status','--porcelain'],text=True).strip()
result = {'revision':revision,'status':'failed' if errors else 'passed','markdown_files':len(files),'local_links':links,
