...
S=ROOT/'extras/der-checkouts/vs2-interaction-semantic3';E=ROOT/'extras/der-evidence/vs2-interaction/r1'
...
if phase=='p1':
    if not S.exists():git('worktree','add','-b','feat/vs2-interaction-final',str(S),BASE,cwd=R)
...
'''
    ...
    header=next(line for line in original(path).splitlines() if '<header>' in line)
    source=(S/path).read_text(encoding='utf-8').splitlines();source=[header if '<header>' in line else line for line in source]
    write(path,'\n'.join(source)+'\n')
    ...
    path='src/semantic_reviewer/web/templates/base.html';lines=(S/path).read_text(encoding='utf-8').splitlines();lines=[next(line for line in original(path).splitlines() if '<header>' in line) if '<header>' in value else value for value in lines];write(path,'\n'.join(lines)+'\n')
