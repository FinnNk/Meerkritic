...
ROOT = Path('WORKSPACE')
E = ROOT/'extras/der-evidence/source-reading/r1'
...
records = []
for label, sha in [('diary-complete', i['diary']), *zip(('p1','p2','p3'), i['commits'])]:
...
assert [v['tests'] for v in records] == [253,241,253,253]
...
save('verification-summary.json', dict(status='passed', base=i['base'], head=i['commits'][-1], checkpoints=records, test_changes=changes, retained_failure='targeted-first.log', failure_disposition='Existing test spy now forwards keyword arguments; selection evidence is checked; numeric GitHub redirects retained and standalone importer bound to its checkout. Failed diagnostics and incomplete rehearsals retained; final checkpoints pass afresh.'))

for name, arguments in [
    ('equivalence.json',['equivalence','--repo',R,'--diary',i['diary'],'--semantic',i['commits'][-1],'--diary-base',i['base'],'--semantic-base',i['base']]),
    ('snapshot.json',['snapshot','--repo',R,'--store',STORE,'--pair','source-reading','--round','r1','--diary-base',i['base'],'--semantic-base',i['base'],'--diary',i['diary'],'--semantic',i['commits'][-1]])
]:
    (E/name).write_text(run([PY,SKILL,*arguments]),encoding='utf-8')
manifest = STORE/'pairs/source-reading/rounds/r1/manifest.json'
actor = dict(client='Codex desktop',cli_version='not exposed',model='GPT-6',session='/root')
review = dict(schema_version=1,pair_id='source-reading',round_id='r1',manifest_sha256=hashlib.sha256(manifest.read_bytes()).hexdigest(),semantic_tip=i['commits'][-1],actor=actor,independence='self-review',mode='full',status='complete',orientation_done=True,reviewed_commits=i['commits'],aggregate_done=True,findings=[],check_records=['review-notes.md','propositions.md','verification-summary.json','test-change-identities.json','equivalence.json','docs-check.json','architecture-delta.json','browser-check.json','docs-render-correction.json','rehearsal3-runtime.json'],limitations=['Author self-review, not independent approval','Windows/Python 3.12 only','No model rerun or human research judgement','Earlier targeted failure retained; cross-tab research-save discrepancy requires separate reconciliation'],carry_forward_record=None,platform_approval=False)
...
event=dict(kind='verification',round_id='r1',actor=actor,payload=dict(stage='locally_prepared',base=i['base'],diary=i['diary'],semantic=i['commits'][-1],records=['source-reading/r1/verification-summary.json','source-reading/r1/review.json'],note='Chronology is retained by materiality.md, discoveries.md and diary commits; this event records completed preparation, not an earlier setup time.'))
save('prepared-event.json',event)
(E/'prepared-ledger.json').write_text(run([PY,SKILL,'record','--repo',R,'--store',STORE,'--pair','source-reading','--event',E/'prepared-event.json','--expected-last','none']),encoding='utf-8')
