...
PY = ROOT/'extras/der-checkouts/assessment-form/.venv/Scripts/python.exe'
...
records = []
for label, sha in [('diary-final', i['diary']), *zip(('p1','p2'), i['commits'])]:
...
assert [v['tests'] for v in records] == [241,241,241]
...
assert len(changes['added']) == 8 and not changes['removed'] and len(changes['altered']) == 1
save('test-change-identities.json', changes)
save('verification-summary.json', dict(status='passed', base=i['base'], head=i['commits'][-1], checkpoints=records, test_changes=changes, retained_failure='form-tests-first.log', failure_disposition='CRLF duplicate matching fixed; styling no longer matches HTML button-absence assertions. Browser error visibility corrected. Further lone-CR review coverage added. Failures retained; final checkpoints pass afresh.'))

for name, arguments in [
    ('equivalence.json',['equivalence','--repo',R,'--diary',i['diary'],'--semantic',i['commits'][-1],'--diary-base',i['base'],'--semantic-base',i['base']]),
    ('snapshot.json',['snapshot','--repo',R,'--store',STORE,'--pair','assessment-form','--round','r1','--diary-base',i['base'],'--semantic-base',i['base'],'--diary',i['diary'],'--semantic',i['commits'][-1]])
]:
    (E/name).write_text(run([PY,SKILL,*arguments]),encoding='utf-8')
manifest = STORE/'pairs/assessment-form/rounds/r1/manifest.json'
actor = dict(client='Codex desktop',cli_version='not exposed',model='GPT-6',session='/root')
review = dict(schema_version=1,pair_id='assessment-form',round_id='r1',manifest_sha256=hashlib.sha256(manifest.read_bytes()).hexdigest(),semantic_tip=i['commits'][-1],actor=actor,independence='self-review',mode='full',status='complete',orientation_done=True,reviewed_commits=i['commits'],aggregate_done=True,findings=[],check_records=['review-notes.md','propositions.md','verification-summary.json','test-change-identities.json','equivalence.json','docs-check.json','architecture-delta.json','browser-check.json'],limitations=['Author self-review, not independent approval','Windows/Python 3.12 only','No model rerun or human research judgement','Earlier targeted failure retained; cross-tab research-save discrepancy requires separate reconciliation'],carry_forward_record=None,platform_approval=False)
...
event=dict(kind='verification',round_id='r1',actor=actor,payload=dict(stage='locally_prepared',base=i['base'],diary=i['diary'],semantic=i['commits'][-1],records=['assessment-form/r1/verification-summary.json','assessment-form/r1/review.json'],note='Chronology is retained by materiality.md, discoveries.md and diary commits; this event records completed preparation, not an earlier setup time.'))
save('prepared-event.json',event)
(E/'prepared-ledger.json').write_text(run([PY,SKILL,'record','--repo',R,'--store',STORE,'--pair','assessment-form','--event',E/'prepared-event.json','--expected-last','none']),encoding='utf-8')
