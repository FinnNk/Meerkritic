...
ROOT=Path('WORKSPACE');E=ROOT/'extras/der-evidence/source-reading/r1';PAIR=ROOT/'extras/der-evidence/pairs/source-reading';PUB=ROOT/'extras/der-publication/source-reading-r1'
...
for round_id in ('r1',):
 ...
 for path in sorted((ROOT/'extras/der-evidence/source-reading'/round_id).iterdir()):
  if path.suffix not in ('.md','.json','.log','.py','.txt') or path.name in ('sample-replay.json','demo.stdout.log','demo.stderr.log'):continue
...
(PUB/'README.md').write_text("""# Preserved source reading: review evidence

This separate archive preserves implementation chronology and three semantic review
...
run(['git','init','-b','evidence/source-reading-r1',PUB])
...
result={'archive_commit':run(['git','-C',PUB,'rev-parse','HEAD']),'archive_branch':'evidence/source-reading-r1','files':len(files),'bytes':sum(p.stat().st_size for p in files)}
