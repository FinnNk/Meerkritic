...
ROOT=Path('WORKSPACE');E=ROOT/'extras/der-evidence/assessment-clarity/r1';PAIR=ROOT/'extras/der-evidence/pairs/assessment-clarity';PUB=ROOT/'extras/der-publication/assessment-clarity-r1';R=ROOT/'extras/der-checkouts/assessment-clarity-semantic';W=ROOT/'working'
PY=ROOT/'extras/der-checkouts/assessment-clarity/.venv/Scripts/python.exe';SKILL=R/'.agents/skills/double-entry-review/scripts/der.py'
def run(args):
...
event={'kind':'publication','round_id':'r1','actor':{'client':'Codex desktop','cli_version':'not exposed','model':'GPT-6','session':'/root'},'payload':{'stage':'owner_review_ready','pr':pr['url'],'head':pr['headRefOid'],'base':pr['baseRefOid'],'archive_commit':archive['archive_commit'],'hosted_qualification':{'required_contexts':[],'observed_rollup':[],'note':'No hosted checks required for this stack base. Owner acceptance remains project policy and main has a separate review gate. No hosted CI run claimed.'},'owner_approval':False,'integrated':False,'evidence':['published-pr.json','host-rules.json','published-refs.txt','host-rules.json','verification-summary.json','review.json','equivalence.json']}}
(E/'ready-event.json').write_text(json.dumps(event,indent=2),encoding='utf-8')
last=json.loads(run([PY,SKILL,'ledger','--store',ROOT/'extras/der-evidence','--pair','assessment-clarity']))['last_hash']
...
result=subprocess.run([str(x) for x in [PY,SKILL,'record','--repo',R,'--store',ROOT/'extras/der-evidence','--pair','assessment-clarity','--event',E/'ready-event.json','--expected-last',last]],env=env,capture_output=True,text=True,encoding='utf-8');assert result.returncode==0,result.stdout+result.stderr
...
for name in ('export-result.json','publish.py','archive-import.log','publish-archive.log','publish-branch.log','publish-pr.log','pr-url.txt','pr-body.md','published-pr.json','host-rules.json','published-refs.txt','ready-event.json','ready-ledger.json','qualify.py'):
 ...
 index.append({'source':'r1/'+name,'source_sha256':hashlib.sha256(raw).hexdigest(),'export_sha256':hashlib.sha256(output).hexdigest(),'transformation':'Local paths replaced; UTF-8 BOM removed if present'})
...
run(['git','-C',PUB,'add','.']);run(['git','-C',PUB,'commit','-m','docs: record verified assessment clarity publication'])
...
run(git+['fetch',PUB,'refs/heads/evidence/assessment-clarity-r1:refs/heads/evidence/assessment-clarity-r1']);run(git+['push','origin','refs/heads/evidence/assessment-clarity-r1'])
sha=run(['git','-C',PUB,'rev-parse','HEAD']);refs=run(git+['ls-remote','origin','refs/heads/feat/vs2-draft-repair','refs/heads/evidence/assessment-clarity-r1','refs/heads/feat/assessment-clarity'])
