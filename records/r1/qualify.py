...
ROOT=Path('WORKSPACE');E=ROOT/'extras/der-evidence/assessment-form/r1';PAIR=ROOT/'extras/der-evidence/pairs/assessment-form';PUB=ROOT/'extras/der-publication/assessment-form-r1';R=ROOT/'extras/der-checkouts/assessment-form-semantic';W=ROOT/'working'
PY=ROOT/'extras/der-checkouts/assessment-form/.venv/Scripts/python.exe';SKILL=R/'.agents/skills/double-entry-review/scripts/der.py'
def run(args):
...
last=json.loads(run([PY,SKILL,'ledger','--store',ROOT/'extras/der-evidence','--pair','assessment-form']))['last_hash']
...
result=subprocess.run([str(x) for x in [PY,SKILL,'record','--repo',R,'--store',ROOT/'extras/der-evidence','--pair','assessment-form','--event',E/'ready-event.json','--expected-last',last]],env=env,capture_output=True,text=True,encoding='utf-8');assert result.returncode==0,result.stdout+result.stderr
...
run(['git','-C',PUB,'add','.']);run(['git','-C',PUB,'commit','-m','docs: record verified assessment form publication'])
...
run(git+['fetch',PUB,'refs/heads/evidence/assessment-form-r1:refs/heads/evidence/assessment-form-r1']);run(git+['push','origin','refs/heads/evidence/assessment-form-r1'])
sha=run(['git','-C',PUB,'rev-parse','HEAD']);refs=run(git+['ls-remote','origin','refs/heads/feat/assessment-clarity','refs/heads/evidence/assessment-form-r1','refs/heads/feat/assessment-form'])
