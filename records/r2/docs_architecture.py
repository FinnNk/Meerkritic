...
ROOT=Path('WORKSPACE'); R=ROOT/'extras/der-checkouts/source-reading'; E=ROOT/'extras/der-evidence/source-reading/r2'; B=ROOT/'extras/der-checkouts/source-reading-check-baseline'
...
for relative in ('docs/development/preserved-source.md','docs/development/annotations.md','docs/development/assessment-fields.md'):
...
record={'status':'passed' if not errors else 'failed','revision':run(['git','rev-parse','HEAD']).strip(),'links':links,'tables':tables,'imported_files_verified':len(manifest['files']),'errors':errors,'cli_help':commands,'rendered':['preserved-source.html','annotations.html','assessment-fields.html']}
