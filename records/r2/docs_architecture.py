...
ROOT=Path('WORKSPACE'); R=ROOT/'extras/der-checkouts/vs2-study-tools-semantic-r2'; E=ROOT/'extras/der-evidence/vs2-study-tools/r2'; B=ROOT/'extras/der-checkouts/vs2-study-tools-check-baseline'
PY=ROOT/'extras/der-checkouts/vs2-study-tools/.venv/Scripts/python.exe'
...
for relative in ('docs/development/study-preparation.md','docs/edr/0001-discovery-grouping-method.md'):
 text=(R/relative).read_text(encoding='utf-8');(E/(Path(relative).stem+'.html')).write_text(md.render(text),encoding='utf-8')
commands=[]
for arguments in (['--help'],['--source','source.json','plan','--help'],['--source','source.json','record','--help'],['--source','source.json','progress','--help']):
 output=run([PY,R/'tools/study_inputs.py',*arguments]);commands.append({'arguments':arguments,'output':output,'exit_code':0})
record={'status':'passed' if not errors else 'failed','revision':run(['git','rev-parse','HEAD']).strip(),'links':links,'tables':tables,'imported_files_verified':len(manifest['files']),'errors':errors,'cli_help':commands,'rendered':['study-preparation.html','0001-discovery-grouping-method.html']}
