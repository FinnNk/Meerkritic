...
ROOT=Path('WORKSPACE');E=ROOT/'extras/der-evidence/vs2-study-tools/r2';PAIR=ROOT/'extras/der-evidence/pairs/vs2-study-tools';PUB=ROOT/'extras/der-publication/vs2-study-tools-r2';R=ROOT/'extras/der-checkouts/vs2-study-tools-semantic-r2';W=ROOT/'working'
...
result=subprocess.run([str(x) for x in [PY,SKILL,'record','--repo',R,'--store',ROOT/'extras/der-evidence','--pair','vs2-study-tools','--event',E/'ready-event.json','--expected-last',last]],env=env,capture_output=True,text=True,encoding='utf-8');assert result.returncode==0,result.stdout+result.stderr
...
index=[]
for name in ('export-result.json','publish.py','archive-import.log','publish-archive.log','publish-branch.log','publish-pr.log','pr-url.txt','pr-body.md','published-pr.json','published-refs.txt','ready-event.json','ready-ledger.json','qualify.py'):
...
run(['git','-C',PUB,'add','.']);run(['git','-C',PUB,'commit','-m','docs: record verified study preparation publication'])
...
run(git+['fetch',PUB,'refs/heads/evidence/vs2-study-tools-r2:refs/heads/evidence/vs2-study-tools-r2']);run(git+['push','origin','refs/heads/evidence/vs2-study-tools-r2'])
sha=run(['git','-C',PUB,'rev-parse','HEAD']);refs=run(git+['ls-remote','origin','refs/heads/main','refs/heads/evidence/vs2-study-tools-r2','refs/heads/feat/vs2-study-tools'])
