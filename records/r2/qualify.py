...
ROOT=Path('WORKSPACE');E=ROOT/'extras/der-evidence/source-reading/r2';PAIR=ROOT/'extras/der-evidence/pairs/source-reading';PUB=ROOT/'extras/der-publication/source-reading-r2';R=ROOT/'extras/der-checkouts/source-reading-semantic';W=ROOT/'working'
PY=ROOT/'extras/der-checkouts/source-reading/.venv/Scripts/python.exe';SKILL=R/'.agents/skills/double-entry-review/scripts/der.py'
def run(args):
...
result=subprocess.run([str(x) for x in [PY,SKILL,'record','--repo',R,'--store',ROOT/'extras/der-evidence','--pair','source-reading','--event',E/'ready-event.json','--expected-last',last]],env=env,capture_output=True,text=True,encoding='utf-8');assert result.returncode==0,result.stdout+result.stderr
...
run(['git','-C',PUB,'add','.']);run(['git','-C',PUB,'commit','-m','docs: record verified source reading publication'])
git=['git','-c','safe.directory='+W.as_posix(),'-C',W]
assert run(git+['ls-remote','origin','refs/heads/evidence/source-reading-r2']).split()[0]==archive['archive_commit']
run(git+['fetch',PUB,'refs/heads/evidence/source-reading-r2:refs/heads/evidence/source-reading-r2']);run(git+['push','origin','refs/heads/evidence/source-reading-r2'])
sha=run(['git','-C',PUB,'rev-parse','HEAD']);refs=run(git+['ls-remote','origin','refs/heads/docs/repair-text-encoding','refs/heads/evidence/source-reading-r2','refs/heads/feat/source-reading'])
