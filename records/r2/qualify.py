...
ROOT=Path('WORKSPACE');E=ROOT/'extras/der-evidence/vs2-comparison/r2';PAIR=ROOT/'extras/der-evidence/pairs/vs2-comparison';PUB=ROOT/'extras/der-publication/vs2-comparison-r2';R=ROOT/'extras/der-checkouts/vs2-comparison-semantic-r2';W=ROOT/'working'
...
last=json.loads(run([PY,SKILL,'ledger','--store',ROOT/'extras/der-evidence','--pair','vs2-comparison']))['last_hash']
...
result=subprocess.run([str(x) for x in [PY,SKILL,'record','--repo',R,'--store',ROOT/'extras/der-evidence','--pair','vs2-comparison','--event',E/'ready-event.json','--expected-last',last]],env=env,capture_output=True,text=True,encoding='utf-8');assert result.returncode==0,result.stdout+result.stderr
...
run(['git','-C',PUB,'add','.']);run(['git','-C',PUB,'commit','-m','docs: record verified grouping comparison publication'])
...
run(git+['fetch',PUB,'refs/heads/evidence/vs2-comparison-r2:refs/heads/evidence/vs2-comparison-r2']);run(git+['push','origin','refs/heads/evidence/vs2-comparison-r2'])
sha=run(['git','-C',PUB,'rev-parse','HEAD']);refs=run(git+['ls-remote','origin','refs/heads/main','refs/heads/evidence/vs2-comparison-r2','refs/heads/feat/vs2-comparison-r2'])
