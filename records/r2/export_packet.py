...
ROOT=Path('WORKSPACE');E=ROOT/'extras/der-evidence/source-reading/r2';PAIR=ROOT/'extras/der-evidence/pairs/source-reading';PUB=ROOT/'extras/der-publication/source-reading-r2'
...
run(['git','init','-b','evidence/source-reading-r2',PUB])
...
result={'archive_commit':run(['git','-C',PUB,'rev-parse','HEAD']),'archive_branch':'evidence/source-reading-r2','files':len(files),'bytes':sum(p.stat().st_size for p in files)}
