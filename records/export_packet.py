...
PAIR=ROOT/'extras/der-evidence/pairs/vs2-rules'
M=PAIR/'rounds/r1/manifest.json'
A=ROOT/'extras/der-checkouts/vs2-rules-archive'
R=ROOT/'extras/der-checkouts/vs2-rules'
PUB=ROOT/'extras/der-publication/vs2-rules-r1'
...
actor={'client':'Codex desktop','cli_version':'not exposed','model':'GPT-6','session':'vs2-rules-2026-09-20'}
commits=[json.loads((E/(p+'-identity.json')).read_text())['revision'] for p in ('p1','p2','p3')]
...
review={'schema_version':1,'pair_id':'vs2-rules','round_id':'r1',
    ...
    'check_records':['verification-summary.json','review-notes.md','live-1b250c59-74e6-4fa5-8267-1d087662eb83.json'],
...
event={'kind':'verification','round_id':'r1','actor':actor,'payload':{
    'stage':'locally_prepared','base':'e81c2e867e714f2fed72d235eb44ba035b4bf302',
    'diary':'ebd6d9eff6d74340ba15266217b6d9ce063d4dbf','semantic':commits[-1],
...
if not (E/'prepared-ledger.json').exists():
    (E/'prepared-ledger.json').write_text(run([PY,str(SKILL),'record','--repo',str(A),
        '--store',str(ROOT/'extras/der-evidence'),'--pair','vs2-rules',
...
(PUB/'README.md').write_text('''# VS2 rule synthesis review evidence
...
run(['git','init','-b','evidence/vs2-rules-r1',str(PUB)])
...
run(['git','-C',str(PUB),'commit','-m','docs: archive VS2 rule synthesis review evidence'])
for path in files:
