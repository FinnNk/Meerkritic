...
ROOT=Path('WORKSPACE')
E=ROOT/'extras/der-evidence/vs2-interaction/r1'
PAIR=ROOT/'extras/der-evidence/pairs/vs2-interaction'
M=PAIR/'rounds/r1/manifest.json'
A=ROOT/'extras/der-checkouts/vs2-interaction-archive'
R=ROOT/'extras/der-checkouts/vs2-interaction'
PUB=ROOT/'extras/der-publication/vs2-interaction-r1'
...
commits=[json.loads((E/(p+'-identity.json')).read_text())['revision'] for p in ('p1','p2','p3','p4','p5')]
...
review={'schema_version':1,'pair_id':'vs2-interaction','round_id':'r1',
    ...
    'check_records':['verification-summary.json','review-notes.md','live-http-release.json','live-guidance-4fdca658-07f9-4029-aace-f33e01ccfbb2.json'],
...
event={'kind':'verification','round_id':'r1','actor':actor,'payload':{
    ...
    'diary':'6bf1544b1fa1f70f188ae19fd08f55df78574787','semantic':commits[-1],
...
(PUB/'README.md').write_text('''# VS2 research interaction review evidence
...
run(['git','init','-b','evidence/vs2-interaction-r1',str(PUB)])
...
run(['git','-C',str(PUB),'commit','-m','docs: archive VS2 research interaction review evidence'])
for path in files:
